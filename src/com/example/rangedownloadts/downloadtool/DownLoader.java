...
public class DownLoader {
	...
	public DownLoader(String urlstr, String localfile, int threadcount,
			Handler mHandler, RangeDownLoadDao dao) {
		this.urlstr = urlstr;
	...
	public class MyTheard extends Thread {
		...
		public void run() {
			...
			try {
				...
				Log.v(Const.MSG_TAG, threadid+"设置断点续传参数Range:" + "bytes="
						+ (startPos + compeleteSize) + "-" + endPos);
				
				// 断点续传特有的文件访问类 理解为：做拼接用
				randomAccessFile = new RandomAccessFile(localfile, "rwd");
				randomAccessFile.seek(startPos + compeleteSize);
				Log.v(Const.MSG_TAG, threadid+"拼接数据前移动坐标到指定位置:"
						+ (startPos + compeleteSize));
				
				fileIs = connection.getInputStream();
				int range = (int) (endPos -(startPos + compeleteSize));
				Log.v(Const.MSG_TAG, threadid+"--->"+range+"="+endPos+"-("+startPos+"+"+compeleteSize+")");
...
//					message.obj = getDownInfo();
				...
				try {
					Log.v(Const.MSG_TAG, "-->线程："+threadid+"下载完成"+getDownInfo().toString());
